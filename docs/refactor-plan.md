...
**`nettop` 每次调用 30 秒。** 只要模块在位，`/api/network/activity` 就阻塞
30 秒以上——比任何调用方的轮询间隔都长。原因是 nettop 默认做名字解析，加 `-n`
降到 0.09 秒。这不是取舍：拿真实输出量过，带解析的 104 条记录里 103 条远端是
IP、1 条是 `*.*` 通配，`-n` 是 223/224，同样比例，interface 和字节数两边都齐全。
那 30 秒什么都没买到。

**状态视图对 PS 附着状态说假话。** `ps_attached` 恒为 `false`，而模块
`AT+CGATT?` 实际返回 `+CGATT: 1`；`imei` 也一直是空。USB AT 状态读取压根没发
这两条命令，字段停在零值。已补上，`AT+CIMI` 和 `AT+CGSN` 都是裸数字应答，所以
读取函数从 `parseUSBATIMSI` 改名为 `parseUSBATBareDigits`，一个函数服务两处。

demo 模式的状态是硬编码字面量，不走这条路径，所以护栏看不到这两处变化——它们
只在真实硬件上可观测，已直接实测确认。demo 那份字面量自身也有
`reg_status: 1` 配 `ps_attached: false` 的不一致，属于展示数据，未动。
